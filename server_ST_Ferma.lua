...
ai = {
    ...
    provider  = tonumber(ini.ai.provider) or 6,
...
local function _renderPage3(DL, WP, cntX, cntY, cntW)
    ...
    sectionTitle(DL, cntX, cy, cntW, u8'\xc8\xc8 \xce\xd2\xc2\xc5\xd2\xdb \xc0\xc4\xcc\xc8\xcd\xd3')
    cy = cy + 20*MDS
    ...
    DL:AddText(imgui.ImVec2(cntX, cy), u32(CLR.textDim), u8'\xd1\xe5\xf0\xe2\xe8\xf1 \xc8\xc8:')
    cy = cy + 16*MDS
    local pbw = (cntW-6*MDS)*0.5
    local selBg = imgui.ImVec4(0.10,0.30,0.16,1)
    local unBg  = imgui.ImVec4(0.08,0.08,0.08,1)
    local hovBg = imgui.ImVec4(0.15,0.15,0.15,1)
    if dlBtn(DL, cntX, cy, pbw, 30*MDS,
        ai.provider==6 and selBg or unBg, hovBg,
        u8'\xd1\xe5\xf0\xe2\xe5\xf0', ai.provider==6 and CLR.green or CLR.textDim, 4*MDS) then
        ai.provider = 6; ai.save()
    end
    if dlBtn(DL, cntX+pbw+6*MDS, cy, pbw, 30*MDS,
        ai.provider==3 and selBg or unBg, hovBg,
        'Groq', ai.provider==3 and CLR.green or CLR.textDim, 4*MDS) then
        ai.provider = 3; ai.save()
    end
    imgui.SetCursorPos(imgui.ImVec2(cntX-WP.x, cy-WP.y))
    imgui.Dummy(imgui.ImVec2(cntW, 30*MDS))
    cy = cy + 30*MDS + 4*MDS
    DL:AddText(imgui.ImVec2(cntX, cy), u32(CLR.green), u8'\xd1\xe5\xf0\xe2\xe5\xf0 \xf0\xe5\xea\xee\xec\xe5\xed\xe4\xf3\xe5\xf2\xf1\xff: \xe1\xe5\xe7 VPN \xe8 \xea\xeb\xfe\xf7\xe0')
    cy = cy + 16*MDS

    if ai.provider == 6 then
        DL:AddText(imgui.ImVec2(cntX, cy), u32(CLR.green), u8'\xca\xeb\xfe\xf7 \xed\xe5 \xed\xf3\xe6\xe5\xed, \xc8\xc8 \xf7\xe5\xf0\xe5\xe7 \xf1\xe5\xf0\xe2\xe5\xf0 STRAND.')
        cy = cy + 18*MDS
    else
    ...
    local tbw = 130*MDS
    if dlBtn(DL, cntX, cy, tbw, 28*MDS,
        imgui.ImVec4(0.10,0.20,0.30,1), imgui.ImVec4(0.16,0.32,0.48,1),
        u8'\xd2\xe5\xf1\xf2', CLR.accent, 4*MDS) then
        ai.testSignal()
    end
    if ai.testMsg ~= '' then
        local tcol = ai.testBusy and CLR.textDim or (ai.testOk and CLR.green or CLR.red)
        local tms = imgui.CalcTextSize(ai.testMsg)
        DL:AddText(imgui.ImVec2(cntX+tbw+10*MDS, cy+(28*MDS-tms.y)*0.5), u32(tcol), ai.testMsg)
    end
    imgui.SetCursorPos(imgui.ImVec2(cntX-WP.x, cy-WP.y))
    imgui.Dummy(imgui.ImVec2(cntW, 28*MDS))
    cy = cy + 28*MDS + 8*MDS

    local aiOn = ai.enabled and ai.activeKey() ~= ''
    ...
    lua_thread.create(function()
        ...
        while true do
            ...
            if autoEat then
                ...
                if autoEat and sampIsLocalPlayerSpawned() then
                    lua_thread.create(function()
                        autoEatWaitSat = true
                        sampSendChat('/satiety')
                        local t = 0
