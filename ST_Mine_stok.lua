...
ai.RELAY_URL   = 'https://strandai.victorstrand250.workers.dev/'
ai.RELAY_TOKEN = 'strand_relay_key_2024'
...
function ai.activeKey()
    ...
    elseif ai.provider == 6 then return 'relay'
...
function main()
    ...
    lua_thread.create(function()
        ...
        while true do
            ...
            if _statsDirty and ((prevMode and not mode) or sinceSave >= 60) then
                pcall(saveStats)
                sinceSave = 0
            end
            prevMode = mode
...
function followRecordedRoute()
    ...
    if oreDist <= 1.5 then
        ...
        setGameKeyState(1, 0)
        setGameKeyState(16, 0)
        if currentMiningTarget ~= nil
            and (math.abs(currentMiningTarget[1] - orePos[1]) > 1.5
              or math.abs(currentMiningTarget[2] - orePos[2]) > 1.5) then
            currentMiningTarget = nil
        end
            ...
            if not _miningStuckRetry and (_miningRetryThread == nil or _miningRetryThread.dead) then
                ...
                _miningRetryThread = lua_thread.create(function()
                    ...
                    if okb then
                        ...
                        local t0 = os.clock()
                        while mode and _miningStuckRetry and os.clock() - t0 < 0.9 do
                    ...
                    if mode and _miningStuckRetry then
...
function goEatAtLarek()
    ...
    lua_thread.create(function()
        ...
        if mode then setGameKeyState(1, -255) end
        larekRunning = false
    end)
end
...
function ai.request(prompt, system, cb)
    lua_thread.create(function()
        ...
        elseif provider == 6 then
            if model == '' then model = 'llama-3.3-70b-versatile' end
            url = ai.RELAY_URL
            headers = {
                ['Content-Type']   = 'application/json',
                ['x-relay-token']  = ai.RELAY_TOKEN,
            }
            body = '{"model":"'..esc(model)..'","max_tokens":150,'
                .. '"messages":[{"role":"system","content":"'..esc(system)..'"},'
                .. '{"role":"user","content":"'..esc(prompt)..'"}]}'
...
function ai.testSignal()
    if ai.testBusy then return end
    if ai.activeKey() == '' then
        ai.testOk  = false
        ai.testMsg = u8'\xed\xe5\xf2 \xea\xeb\xfe\xf7\xe0'
        return
    end
    ai.testBusy = true
    ai.testOk   = false
    ai.testMsg  = u8'\xef\xf0\xee\xe2\xe5\xf0\xea\xe0...'
    ai.request('ping', 'Reply with one short word.', function(ok, text)
        ai.testBusy = false
        ai.testOk   = ok
        if ok then
            ai.testMsg = u8'\xf1\xe2\xff\xe7\xfc \xe5\xf1\xf2\xfc'
        else
            ai.testMsg = u8'\xee\xf8\xe8\xe1\xea\xe0: ' .. tostring(text):sub(1, 60)
        end
    end)
end
...
local function renderCollisionTab()
    ...
    imgui.Dummy(imgui.ImVec2(0, 10))
    renderSectionHeader(u8"\xc8\xc8 \xee\xf2\xe2\xe5\xf2\xfb \xe0\xe4\xec\xe8\xed\xf3")

    imgui.TextColored(imgui.ImVec4(0.62,0.57,0.48,1), u8"\xd1\xe5\xf0\xe2\xe8\xf1 \xc8\xc8:")
    local _selC = imgui.ImVec4(0.16,0.40,0.22,1)
    local _unC  = imgui.ImVec4(0.14,0.14,0.14,1)
    local _selH = imgui.ImVec4(0.20,0.50,0.28,1)
    local _unH  = imgui.ImVec4(0.22,0.22,0.22,1)
    imgui.PushStyleColor(imgui.Col.Button,        ai.provider==6 and _selC or _unC)
    imgui.PushStyleColor(imgui.Col.ButtonHovered, ai.provider==6 and _selH or _unH)
    imgui.PushStyleColor(imgui.Col.ButtonActive,  ai.provider==6 and _selC or _unC)
    if imgui.Button(u8'\xd1\xe5\xf0\xe2\xe5\xf0', imgui.ImVec2(110, 0)) then ai.provider = 6 ai.save() end
    imgui.PopStyleColor(3)
    imgui.SameLine()
    imgui.PushStyleColor(imgui.Col.Button,        ai.provider==3 and _selC or _unC)
    imgui.PushStyleColor(imgui.Col.ButtonHovered, ai.provider==3 and _selH or _unH)
    imgui.PushStyleColor(imgui.Col.ButtonActive,  ai.provider==3 and _selC or _unC)
    if imgui.Button('Groq', imgui.ImVec2(110, 0)) then ai.provider = 3 ai.save() end
    imgui.PopStyleColor(3)
    imgui.TextColored(imgui.ImVec4(0.45,0.70,0.45,1), u8"\xd1\xe5\xf0\xe2\xe5\xf0 \xf0\xe5\xea\xee\xec\xe5\xed\xe4\xf3\xe5\xf2\xf1\xff: \xe1\xe5\xe7 VPN \xe8 \xea\xeb\xfe\xf7\xe0")

    if ai.provider == 6 then
        imgui.TextColored(imgui.ImVec4(0.45,0.70,0.45,1), u8"\xca\xeb\xfe\xf7 \xed\xe5 \xed\xf3\xe6\xe5\xed, \xc8\xc8 \xf0\xe0\xe1\xee\xf2\xe0\xe5\xf2 \xf7\xe5\xf0\xe5\xe7 \xf1\xe5\xf0\xe2\xe5\xf0 STRAND.")
    else
    ...
    imgui.Dummy(imgui.ImVec2(0, 4))
    imgui.PushStyleColor(imgui.Col.Button,        imgui.ImVec4(0.10,0.20,0.30,1))
    imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.16,0.32,0.48,1))
    imgui.PushStyleColor(imgui.Col.ButtonActive,  imgui.ImVec4(0.10,0.20,0.30,1))
    if imgui.Button(u8'\xd2\xe5\xf1\xf2', imgui.ImVec2(130, 0)) then
        ai.testSignal()
    end
    imgui.PopStyleColor(3)
    if ai.testMsg ~= '' then
        imgui.SameLine()
        local tcol = ai.testBusy and imgui.ImVec4(0.6,0.6,0.6,1)
                  or (ai.testOk and imgui.ImVec4(0.45,0.85,0.40,1) or imgui.ImVec4(0.90,0.34,0.22,1))
        imgui.TextColored(tcol, ai.testMsg)
    end
